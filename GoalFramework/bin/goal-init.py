...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Initialize Goal Framework in a project."
    )
    parser.add_argument("--project-path", "-p", default=".")
    parser.add_argument(
        "--agent",
        choices=("auto", "codex", "cursor", "opencode", "manual"),
        default="auto",
    )
...
def select_agent(requested: str, available: dict[str, str | None]) -> str:
    if requested != "auto":
        return requested
    return next(
        (name for name in ("opencode", "codex", "cursor") if available[name]), "manual"
    )


def run_agent(agent: str, command: str, prompt: str, cwd: Path) -> bool:
    args = {
        "codex": [command, "exec", prompt],
        "cursor": [command, "--print", prompt],
        "opencode": [command, "run", prompt],
    }[agent]
    try:
...
def main() -> int:
    ...
    if not agents_path.exists() and not args.skip_native_init and agent != "manual":
        ...
        if not args.dry_run:
            ...
            if not command:
                warn(
                    f"未找到 {agent} 命令。将继续创建 Goal Framework；请稍后手动执行该终端的 /init。"
                )
            elif agent == "opencode":
    ...
    if not args.no_agent_prompt:
        if agent == "manual":
            status(
                "未检测到可调用 Agent。请将 templates/goal-framework-init-prompt.md 的内容粘贴到你的 Agent 终端。"
            )
        elif args.dry_run:
            status(f"将调用 {agent} 填写 PROJECT.md 和 CURRENT.md。")
        else:
            command = available[agent]
            prompt = (TEMPLATES / "goal-framework-init-prompt.md").read_text(
                encoding="utf-8"
            )
            if command and not run_agent(agent, command, prompt, project):
                warn(
                    "请在所选 Agent 中手动执行 templates/goal-framework-init-prompt.md 的内容。"
                )

    status(
        "完成。请检查 AGENTS.md、docs/PROJECT.md、docs/CURRENT.md 与项目级 skill 后提交项目变更。"
    )
    return 0
