...
- 推进活跃目标时主动识别对话反馈与目标、关联技术方案的实质冲突和新增要求，提供采用新约定并同步、保留原约定、继续讨论三个选择。
- 用户确认同步即授权更新相关文档，无需追加留档指令；仅要求按新方案实施仍须确认同步，已授权的同一变更不重复询问。
- 统一受管 Agent 规则、operator skill、初始化提示和目标模板，明确完成条件重新验证、部分失败处理与 doctor 的语义检查边界。
- 无新增 CLI 或文档结构；旧项目通过 goal-update 获取规则，已有目标、技术方案和项目状态保持不变。
