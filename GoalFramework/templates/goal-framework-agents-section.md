...
普通的方案讨论、评审或头脑风暴不代表用户授权写文件。当用户明确要求保存、留档、落盘或归档技术方案（或表达等价意图）时，必须完成以下流程：
...
3. 除非用户明确要求更新已有方案，否则不得覆盖已有文档；发生重名时使用新的描述性文件名。
...
- 技术方案使用自由格式 Markdown；仅按上述明确留档流程创建或修改，不预建空文档。
